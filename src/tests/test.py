...
# Input
if True:
    SRC_IMAGES_DESCRIPTION_TXT = ROOT + 'src/tests/valid_images.txt'
    SRC_IMAGES_FOLDER = ROOT + 'src/tests/img/'

# Output
if True:
    # This txt will store image info, such as index, action label, filename, etc.
    # This file is saved but not used.
    DST_IMAGES_INFO_TXT = ROOT + 'src/tests/images_info.txt'

    # Each txt will store the skeleton of each image
    DST_DETECTED_SKELETONS_FOLDER = ROOT + 'src/tests/skeleton_res/'

    # Each image is drawn with the detected skeleton
    DST_VIZ_IMGS_FOLDER = ROOT + 'src/tests/image_viz/'
